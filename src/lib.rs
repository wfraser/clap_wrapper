    ...
    for field in fields {
        ...
        for attr in &mut field.attrs {
            ...
            if head == "clap" {
                return Err(Error::new_spanned(
                    field,
                    "do not use the #[clap] attribute on fields; use #[arg] or #[command] instead",
                ));
            } else if head != "arg" {
                continue;
            }
            ...
            for expr in &mut exprs {
                match expr {
                    // long = "foo"
                    Expr::Assign(expr) if expr.left.to_token_stream().to_string() == "long" => {
                        let renamed = expr_str_lit(&expr.right)?;
                        let name = format!("{prefix}.{renamed}");
                        *expr = syn::parse_quote_spanned! { expr.span() => long = #name };
                    }
                    // long("foo")
                    Expr::Call(expr) if expr.func.to_token_stream().to_string() == "long" => {
    ...
    for attr in &input.attrs {
        ...
        for expr in exprs {
            ...
            name_style = match norm.as_str() {
                ...
                _ => return Err(Error::new_spanned(expr.right, "unrecognized rename style")),
            }
        }
    }
